...
-- Callback
lib.callback.register('DX-HIGH:server:CheckOwnerVehicle', function(_, oldPlate)
...
-- Event
RegisterNetEvent('DX-HIGH:server:updatePlate', function(netID, oldPlate, newPlate)
    local src = source
    ...
    if not checkPlate[1] then 
        MySQL.query('SELECT plate, mods FROM player_vehicles WHERE plate = ?', {oldPlate},function(result)
            if result[1] then
                ...
                TriggerClientEvent('DX-HIGH:client:libNotify', src , Config.Lang['notify'].newlicenseplate ..newPlate, 'success')
                Player.Functions.RemoveItem('licenseplate', 1)
            end
        end)
    else
        TriggerClientEvent('DX-HIGH:client:libNotify', src , Config.Lang['notify'].sameplate, 'error')
    end
end)
...
QBCore.Functions.CreateUseableItem('licenseplate', function(source)
    TriggerClientEvent('DX-HIGH:client:Menu',source)
end)
